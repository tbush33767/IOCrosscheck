...
# ===========================================================================
# enrich_results — Rack-Only upgrade
# ===========================================================================

class TestEnrichRackOnlyUpgrade:
    """L5X alias upgrades Both (Rack Only) → Both with actual tag name."""

    def test_rack_only_upgraded_to_both(self):
        data = _make_data(alias_tags=[
            _alias("TF655", "Rack0:I.Data[5].0", "Temperature Transmitter 655"),
        ])
        enrichment = extract_l5x_enrichment(data)

        result = _match_result(
            plc_name="Rack01", specifier="",
            dev_tag="TF655", plc_address="Rack0:I.Data[5].0",
            classification=Classification.RACK_ONLY,
        )
        result.confidence = Confidence.PARTIAL
        result.strategy_id = 3
        ...
        assert result.plc_tag.name == "TF655"
        assert result.plc_tag.specifier == "Rack0:I.Data[5].0"
        assert result.plc_tag.description == "Temperature Transmitter 655"
        assert "L5X" in result.sources
        assert any("upgraded Rack Only" in s for s in result.audit_trail)

    def test_rack_only_no_alias_stays_rack_only(self):
        """No L5X alias for this address — stays Rack Only."""
        data = _make_data(alias_tags=[])
        enrichment = extract_l5x_enrichment(data)
        ...
        assert result.plc_tag.name == "Rack01"

    def test_non_rack_only_not_upgraded(self):
        """Results that are already Both should not be re-upgraded."""
        data = _make_data(alias_tags=[
            _alias("AN601_EV", "Rack14:O.Data[3].2"),
        ])
        enrichment = extract_l5x_enrichment(data)
        ...
        result = _match_result(
            plc_name="AN601_EV", specifier="Rack14:O.Data[3].2",
            dev_tag="AN601", plc_address="Rack14:O.Data[3].2",
            classification=Classification.BOTH,
        )
        enrich_results([result], enrichment)

        # Should stay Both, not be touched by upgrade logic
        assert result.classification == Classification.BOTH
        assert result.plc_tag.name == "AN601_EV"
